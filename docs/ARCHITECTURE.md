...
*Implements the product mechanism defined in `PRODUCT.md` (mechanism D: reserve-the-slot, confirm-price-near-reservation, closed-set sealed-bid tiebreaker). Written against the founder's proven stack: SvelteKit (Svelte 5) + Supabase + Stripe Connect, Hetzner only if actually needed. Assumes solo/small-team execution — every section optimizes for "fewest moving parts that are still correct," not maximal architecture.*
...
- RLS: **enabled**. Public `select` where `status = 'open'` (browse). Owner (`creator_id`) or an authorized manager (via `manager_creator_links`, checked by a helper function — Section 3) can `insert`/`update`/`delete` while `status in ('draft','open')`. Once `status` leaves `open`, only the escrow/reservation workflow (via `security definer` functions, not raw `update`) can move it further — prevents a creator or manager editing a listing out from under an active reservation.
...
### `deal_tiebreaks` (supports the closed-set sealed-bid mechanic)
- `id uuid PK`
- `listing_id uuid references creator_listings(id)`
...
### `deals` (the confirmed contract)
- `id uuid PK`
- `reservation_id uuid references reservations(id)` (1:1 once confirmed)
...
- RLS: **enabled**. Visible to `creator_id`, `advertiser_id`, linked manager, nobody else.
...
`profiles (creator)` 1—N `creator_listings` 1—N `reservations` N—1 `profiles (advertiser)`; `reservations` 1—1 `deals` 1—N `escrow_transactions`; `manager_creator_links` is the N:N join between manager and creator profiles that everything delegated hangs off of; `audit_log` fans in from every mutating RPC.
...
This is **not** a single PaymentIntent per deal — it's a sequence, because the amounts and destinations aren't known in full at reservation time (final price isn't set until creator confirmation, and the platform fee/manager commission are only computable once that price exists).
...
- **pg_cron** runs a scheduled job every 5 minutes (`select cron.schedule(...)`) that does the *detection* in plain SQL, entirely inside Postgres, no network hop needed for the common cases:
  - `reservations` where `status = 'held' and confirmation_deadline < now()` → call `expire_reservation(id)` (refunds deposit via a queued Stripe call, see below, sets listing back to `open`).
  - `creator_listings` where `status = 'open' and reservation_deadline < now()` → `status = 'expired'`.
  - `deals` where `status = 'active' and delivery_confirmed_or_flagged = true` (or "delivered" state) `and auto_release_at < now() and status != 'disputed'` → call `release_delivery_balance(id)`.
  - `deal_tiebreaks` where `status = 'open' and closes_at < now()` → resolve (pick highest sealed bid among `tiebreak_bids`, mark winner/losers, trigger refunds for losers).
...
**Realtime-necessary events** (Supabase Realtime, Postgres change subscriptions, scoped by the same RLS the tables already have — no separate authorization layer needed for Realtime):
- Creator dashboard: new reservation on one of their listings (`reservations` insert where `listing_id` in their listings) — this is the highest-value realtime moment, it starts their confirmation clock.
- Advertiser: reservation status change (`held → confirmed`, `→ lost_tiebreak`, `→ expired`) and `escrow_transactions` updates (deposit succeeded/failed).
- Manager dashboard: roster-wide feed of the above across all `active`-linked creators (same subscription, filtered via the `is_authorized_for_creator` relationship — Realtime respects RLS so this is a `select`-policy problem, not a new mechanism).
- Confirmation-window countdown ("closing in 3h") is a client-side timer against `confirmation_deadline`, not a server push — no need to push every tick.
